...
from portcullis.report import render_markdown, render_terminal
...
@click.option("--format", "fmt", type=click.Choice(["terminal", "markdown"]),
...
def scan(path: Path, fmt: str, output: Path | None, min_severity: str,
    ...
    if fmt == "markdown":
        text = render_markdown(result, min_severity=threshold)
        if output:
        ...
        if output:
            click.echo("Note: --output is only used with --format markdown.", err=True)
